...
const buildSaveRequestJson = props => {
  const json = {};
  json.user = props.user;
  json.friend = props.friend;
  return json;
};

module.exports.friendExist = async props => {
  try {
    const frdExist = Friends
    .findOne({ user: props.user, friend: props.friend})
    return frdExist
  } catch (error) {
    throw error;
  }
...
module.exports.getUserFriends = async (userId, props) => {
  try {
    ...
    const users = Friends
    .find({ user: userId}).select('friend')
    .populate('friend')
    .limit(Number(limit))
    .skip(Number(skip))
    .exec();
